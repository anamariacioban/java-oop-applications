...
public class Ex4StudentRecordManager {
    ...
    public static void main (String[] args) {
        ...
        // 1. Creăm un scanner pentru a citi inputul utilizatorului.
            Scanner scanner = new Scanner(System.in);
            List<String[]> studentRecords = new ArrayList<>();

            // 2. Întrebăm utilizatorul să introducă înregistrările studenților
    ...
    public static void displayRecords(String[][] records) {
        ...
        for (int i = 0; i < records.length; i++) {
            String[] record = records[i];  // Accesăm fiecare element (un array de string-uri)
            System.out.println(record[0] + "\t" + record[1] + "\t" + record[2]);  // Afișăm valorile
        }
    }
    ...
    public static boolean isInteger(String str) {
        ...
        // 2. Return true if successful, false if it causes an exception
            try {
                Integer.parseInt(str); //incercam sa convertim string in intreg
                return true;
